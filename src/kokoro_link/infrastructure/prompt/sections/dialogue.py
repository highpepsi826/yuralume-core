...
def _render_response_format_instruction(frame: PresenceFrame) -> str:
    ...
    return (
        ...
        "`*沉默片刻*`）。角色此刻能感知到的環境與感官細節也請用星號包住，"
        "讓前端可以把動作/狀態和口語區分渲染。星號 `*...*` 內的動作、表情與狀態描寫"
        "也屬於玩家可見自然語言；環境與感官細節若放在星號內，也同樣必須和台詞使用上方"
        "指定的主要語言。不要因為下方格式範例是中文就把動作描寫寫成中文。"
        "若主要語言是 en-US，動作描寫也應自然寫成英文（例如 `*sets the phone down*`），"
        "而不是 `*放下手機*`。不要改用括號、方括號、破折號或其他符號——全場統一用 "
        "`*...*`。依場面需要，可以寫出數個連續的具體動作、空間距離、聲音、光線、氣味、"
        "觸感與停頓，讓角色的反應有清楚的過程，不要只用一個動詞帶過。台詞與動作可以交錯；"
        "每個 `*...*` 段落請保持單一行，分成數個適中的段落，不要把整篇回覆包在同一組星號裡。"
        "描寫只使用角色當下已知、能感知的細節；不要為了增加描寫新增不必要的長期事實、"
        "人物、地點或約定，也不要替玩家虛構未宣告的行動、表情、位置或想法。"
    )
