...
def _promise(
    *,
    ...
    return PendingFollowUp.new_promise(
        ...
        promise_intent="提醒使用者準備晚上活動",
        scheduled_for=datetime(2026, 8, 18, 12, 0, tzinfo=UTC),
...
async def test_sql_repository_concurrent_adds_converge_on_one_row(
    ...
    real_find = module._find_open_by_dedupe_key
    barrier = asyncio.Barrier(2)

    async def synchronized_find(session, dedupe_key):  # noqa: ANN001
        result = await real_find(session, dedupe_key)
        if result is None:
            await barrier.wait()
        return result

    monkeypatch.setattr(module, "_find_open_by_dedupe_key", synchronized_find)
